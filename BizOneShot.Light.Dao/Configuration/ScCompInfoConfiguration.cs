...
namespace BizOneShot.Light.Dao.Configuration
{
    ...
    internal partial class ScCompInfoConfiguration : EntityTypeConfiguration<ScCompInfo>
    {
        ...
        public ScCompInfoConfiguration(string schema)
        {
            ...
            // Foreign keys
            HasOptional(a => a.ScUsr_RegId).WithMany(b => b.ScCompInfoes_RegId).HasForeignKey(c => c.RegId); // FK_SC_USR_TO_SC_COMP_INFO
            HasOptional(a => a.ScUsr_UpdId).WithMany(b => b.ScCompInfoes_UpdId).HasForeignKey(c => c.UpdId); // FK_SC_USR_TO_SC_COMP_INFO2
            InitializePartial();
        }
