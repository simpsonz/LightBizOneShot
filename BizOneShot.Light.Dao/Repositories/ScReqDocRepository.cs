...
namespace BizOneShot.Light.Dao.Repositories
{
    ...
    {
        Task<IPagedList<ScReqDoc>> GetPagedListSendDocsAsync(int page, int pageSize, string senderId, string sendExpertType, DateTime? startDate = null, DateTime? endDate = null);
        Task<IPagedList<ScReqDoc>> GetPagedListReqDocsAsync(int page, int pageSize, string receiverId, string sendExpertType, DateTime? startDate = null, DateTime? endDate = null);
    ...
    public class ScReqDocRepository : RepositoryBase<ScReqDoc>, IScReqDocRepository
    {
        ...
        public async Task<IPagedList<ScReqDoc>> GetPagedListSendDocsAsync(int page, int pageSize, string senderId, string sendExpertType, DateTime? startDate = null, DateTime? endDate = null)
        {
            return await DbContext.ScReqDocs
                ...
                .Where(rd => rd.SenderId == senderId && rd.Status == "N" && rd.ScUsr_ReceiverId.UsrType == "P" && rd.ScUsr_ReceiverId.UsrTypeDetail == sendExpertType)
                .Where(rd => rd.ReqDt >= startDate && rd.ReqDt <= endDate)
                .OrderByDescending(rd => rd.ReqDt).ToPagedListAsync(page, pageSize);
        }


        public async Task<IPagedList<ScReqDoc>> GetPagedListReqDocsAsync(int page, int pageSize, string receiverId, string sendExpertType, DateTime? startDate = null, DateTime? endDate = null)
        {
            return await DbContext.ScReqDocs
                .Include("ScUsr_ReceiverId")
                .Include("ScUsr_SenderId")
                .Include("ScUsr_SenderId.ScCompInfo")
                .Include("ScUsr_ReceiverId.ScCompInfo")
                .Where(rd => rd.ReceiverId == receiverId && rd.Status == "N" && rd.ScUsr_SenderId.UsrType == "P" && rd.ScUsr_SenderId.UsrTypeDetail == sendExpertType)
                .Where(rd => rd.ReqDt >= startDate && rd.ReqDt <= endDate)
                .OrderByDescending(rd => rd.ReqDt).ToPagedListAsync(page, pageSize);
        }
        ...
        {
            return this.DbContext.ScReqDocs.Add(scReqDoc);
        }
    }
}
