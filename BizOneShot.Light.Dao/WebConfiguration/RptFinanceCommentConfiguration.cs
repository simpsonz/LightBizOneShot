    ...
    internal partial class RptFinanceCommentConfiguration : EntityTypeConfiguration<RptFinanceComment>
    {
        ...
        public RptFinanceCommentConfiguration(string schema)
        {
            ...
            Property(x => x.BasicYear)
                ...
                .HasColumnType("int")
                .HasDatabaseGeneratedOption(DatabaseGeneratedOption.None);
            Property(x => x.BasicMonth)
                .HasColumnName("BASIC_MONTH")
                .IsRequired()
                .HasColumnType("int")
                .HasDatabaseGeneratedOption(DatabaseGeneratedOption.None);
            ...
            // Foreign keys
            HasRequired(a => a.ScBizWork).WithMany(b => b.RptFinanceComments).HasForeignKey(c => c.BizWorkSn);
                // FK_SC_BIZ_WORK_TO_RPT_FINANCE_COMMENT
            HasRequired(a => a.ScCompInfo).WithMany(b => b.RptFinanceComments).HasForeignKey(c => c.CompSn);
                // FK_SC_COMP_INFO_TO_RPT_FINANCE_COMMENT
            HasRequired(a => a.ScUsr).WithMany(b => b.RptFinanceComments).HasForeignKey(c => c.ExpertId);
                // FK_SC_USR_TO_RPT_FINANCE_COMMENT
            InitializePartial();
        }
