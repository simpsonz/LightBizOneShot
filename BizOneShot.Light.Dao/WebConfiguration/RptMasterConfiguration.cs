...
namespace BizOneShot.Light.Dao.WebConfiguration
{
    ...
    internal partial class RptMasterConfiguration : EntityTypeConfiguration<RptMaster>
    {
        ...
        public RptMasterConfiguration(string schema)
        {
            ...
            // Foreign keys
            HasOptional(a => a.ScUsr).WithMany(b => b.RptMasters).HasForeignKey(c => c.MentorId);
                // FK_SC_USR_TO_RPT_MASTER
            HasRequired(a => a.QuesMaster).WithMany(b => b.RptMasters).HasForeignKey(c => c.QuestionSn);
                // FK_QUES_MASTER_TO_RPT_MASTER
            HasRequired(a => a.ScBizWork).WithMany(b => b.RptMasters).HasForeignKey(c => c.BizWorkSn);
                // FK_SC_BIZ_WORK_TO_RPT_MASTER
            HasRequired(a => a.ScCompInfo).WithMany(b => b.RptMasters).HasForeignKey(c => c.CompSn);
                // FK_SC_COMP_INFO_TO_RPT_MASTER
            InitializePartial();
        }
